...
from .models_tf import ECMLP, Conv2D, Linear, LayerNorm, Autoencoder, Flatten
...
def dim2int(dim):
    ''' extract value from Dimension object hack '''
    if not isinstance(dim, int):
        return dim.value
    return dim


class ResNetTF(ECMLP):
    """ResNet model."""
    ...
    def _build_model(self, x):
        ...
        with tf.variable_scope('dae'):
            x = self._flatten('flatten_ae_in', x)
            #XXX This is the DAE -- change params right here
            x = self._dae('latent_dae', x, 512, activation=tf.tanh, noise_std=0.1)
    ...
    def _dae(self, name, x, num_hidden, activation, noise_std=0.1):
        """DAE."""
        if self.init_layers:
            ae = Autoencoder(num_hidden, activation=activation)
            ae.name = name
            self.layers += [ae]
            input_shape_ = tuple(dim2int(v) for v in x.shape)
            ae.set_input_shape(input_shape_)
        else:
            ae = self.layers[self.layer_idx]
            self.layer_idx += 1
        ae.device_name = self.device_name
        ae.set_pre_state(x)
        x = x + tf.cast(tf.random_normal(shape=tf.shape(x), stddev=noise_std), tf.float32)
        x = ae.fprop(x)
        ae.set_post_state(x)
        return x

    def _flatten(self, name, x):
        """DAE."""
        if self.init_layers:
            flat = Flatten()
            flat.name = name
            self.layers += [flat]
            input_shape_ = tuple(dim2int(v) for v in x.shape)
            flat.set_input_shape(input_shape_)
        else:
            flat = self.layers[self.layer_idx]
            self.layer_idx += 1
        flat.device_name = self.device_name
        x = flat.fprop(x)
        return x
    ...
    def _conv(self, name, x, filter_size, in_filters, out_filters, strides):
        """Convolution."""
        if self.init_layers:
            conv = Conv2D(out_filters, (filter_size, filter_size),
                          strides[1:3], 'SAME', w_name='DW')
            conv.name = name
            self.layers += [conv]
            input_shape = None, filter_size, filter_size, dim2int(in_filters)
            conv.set_input_shape(input_shape)
        else:
    ...
    def _fully_connected(self, x, out_dim):
        ...
        if self.init_layers:
            fc = Linear(out_dim, w_name='DW')
            fc.name = 'logits'
            self.layers += [fc]
            input_shape_ = (dim2int(v) for v in x.shape)
            fc.set_input_shape(input_shape_)
        else:
